...
html+=`<div class="flex flex-col"><div class="card-micro ${cor}" onclick="abrirItem('${itemClick}')"><span class="percent-big">${media}%</span><span style="font-size:10px;font-weight:900;color:#000000;">${k}</span></div><div style="font-size:7px;font-weight:700;color:#000000;margin-top:2px;padding:2px;text-align:center;line-height:1.1;">${descricao}</div><div style="font-size:6px;font-weight:900;color:#0f172a;text-align:center;line-height:1.1;margin-top:1px;">${responsavel}</div></div>`
})
...
box.innerHTML=''
lista.innerHTML=''
let dados=[...allData].filter(i=>getTotal(i)>=100).sort(compareSubitem)
